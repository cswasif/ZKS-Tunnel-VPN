    ...
    S: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin,
{
    ...
    let commit_json = commit_event.to_json()
        .map_err(|e| format!("Failed to serialize commitment: {}", e))?;
    
    writer.send(Message::Text(commit_json)).await
    ...
    while !entropy_tax.all_committed(&peer_ids) {
        if let Some(Ok(Message::Text(msg))) = reader.next().await {
            if let Ok(event) = EntropyEvent::from_json(&msg) {
                match event {
                    EntropyEvent::Commit { peer_id, commitment: commit_hex } => {
                        if peer_ids.contains(&peer_id) && !committed_peers.contains(&peer_id) {
                            ...
                            debug!("✅ Received commitment from {} ({}/{})", 
                                peer_id, committed_peers.len(), peer_ids.len());
                        }
                    }
    ...
    writer.send(Message::Text(reveal_json)).await
    ...
    while !entropy_tax.all_revealed(&peer_ids) {
        if let Some(Ok(Message::Text(msg))) = reader.next().await {
            if let Ok(event) = EntropyEvent::from_json(&msg) {
                match event {
                    EntropyEvent::Reveal { peer_id, entropy: entropy_hex } => {
                        if peer_ids.contains(&peer_id) && !revealed_peers.contains(&peer_id) {
                            ...
                            debug!("✅ Received entropy from {} ({}/{})", 
                                peer_id, revealed_peers.len(), peer_ids.len());
                        }
                    }
